module dglc_datamode_store_mod
   ...
   public  :: dglc_datamode_store_advertise
   public  :: dglc_datamode_store_init_pointers
   public  :: dglc_datamode_store_advance
   public  :: dglc_datamode_store_restart_write
   public  :: dglc_datamode_store_restart_read

   logical  :: initialized_store = .false.
...
!===============================================================================
   ...
   subroutine dglc_datamode_store_advertise(NStateExp, fldsexport, NStateImp, fldsimport, &
        flds_scalar_name, rc)
   ...
   end subroutine dglc_datamode_store_advertise

   !===============================================================================
   subroutine dglc_datamode_store_init_pointers(NStateExp, NstateImp, rc)
   ...
   !===============================================================================
   subroutine dglc_datamode_store_advance(gcomp, pio_subsystem, io_type, io_format, &
     ...
     ! input/output variables
      ...
      ! local variables
      type(ESMF_FieldBundle) :: fldbun_store
      type(ESMF_DistGrid)    :: distgrid
      type(ESMF_Field)       :: field_store
      type(ESMF_VM)          :: vm
      ...
      character(len=*), parameter :: subname='(dglc_datamode_store_advance): '
      !-------------------------------------------------------------------------------

      rc = ESMF_SUCCESS

      if (.not. initialized_store) then
         ...
         do ns = 1,num_icesheets
            ...
            fldbun_store = ESMF_FieldBundleCreate(rc=rc) ! input field bundle

            ! "ice thickness" ;
            field_store = ESMF_FieldCreate(model_meshes(ns), ESMF_TYPEKIND_R8, &
                 name='thk', meshloc=ESMF_MESHLOC_ELEMENT, rc=rc)
            if (chkerr(rc,__LINE__,u_FILE_u)) return
            call ESMF_FieldBundleAdd(fldbun_store, (/field_store/), rc=rc)
            if (chkerr(rc,__LINE__,u_FILE_u)) return
            ...
            call dshr_fldbun_getFldPtr(fldbun_store, 'topg', topog, rc=rc)
            if (ChkErr(rc,__LINE__,u_FILE_u)) return
            rcode = pio_inq_varid(pioid, 'topg', varid)
            call pio_read_darray(pioid, varid, pio_iodesc, topog, rcode)

            call dshr_fldbun_getFldPtr(fldbun_store, 'thk', thck, rc=rc)
            if (ChkErr(rc,__LINE__,u_FILE_u)) return
            rcode = pio_inq_varid(pioid, 'thk', varid)
   ...
   end subroutine dglc_datamode_store_advance
  ...
  !===============================================================================
  subroutine dglc_datamode_store_restart_write(model_meshes, case_name, &
    ...
    do ns = 1,num_icesheets
       ! Need to explicitly write restart since store mode does not read a stream
       write(cnum,'(i0)') ns
  ...
  end subroutine dglc_datamode_store_restart_write

  !===============================================================================
  subroutine dglc_datamode_store_restart_read(model_meshes, restfilem, rpfile, &
    ...
    integer             :: rcode
    integer             :: tmp(1)
    character(*), parameter :: subName = "(dglc_datamode_store_restart_read) "
    !-------------------------------------------------------------------------------
 ...
 end subroutine dglc_datamode_store_restart_read

end module dglc_datamode_store_mod
